...
if (val.length > 2) {
  try {
    const res = await fetch(`https://api.coingecko.com/api/v3/search?query=${val}`);
...
const selectToken = (coin) => {
setSearchTerm(coin.name + ' (' + coin.symbol.toUpperCase() + ')');
setSelectedTokenId(coin.id);
setSearchResults([]);
};

const generarPrompt = () => {
const tokenName = searchTerm ? searchTerm.toUpperCase() : "el token seleccionado";
...
const tokenIdToFetch = selectedTokenId ? selectedTokenId : searchTerm.toLowerCase().replace(/\s+/g, '-');

try {
  const res = await fetch('/api/prices?token=' + tokenIdToFetch + '&days=60');
  const data = await res.json();
...
<header className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 gap-4 relative z-50">
...
<div className="absolute w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden z-50">
...
<span className="font-bold text-slate-800 text-sm">{coin.name}</span>
...
{loading ? "Calculando..." : "Analizar"}
  ...
  <Tabs.Root defaultValue="fundamental" className="w-full relative z-10">
